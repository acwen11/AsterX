...
wenozp(T gf_Imm, T gf_Im, T gf_I, T gf_Ip, T gf_Ipp, const CCTK_REAL dx,
       T weno_eps, bool mp) {
  ...
  const vec<T, 3> dj{gf_Imm - 2.0 * gf_Im + gf_I, gf_Im - 2.0 * gf_I + gf_Ip,
  ...
  const vec<T, 3> betaZ{
      ...
      (13.0 / 12.0) * pow2(dj(1)) + (1.0 / 4.0) * pow2(gf_Im - gf_Ip),
  ...
  if (mp) {
    ...
    const vec<T, 4> mp_p{4.0 * dj(1) - dj(2), 4.0 * dj(2) - dj(1), dj(1),
                         dj(2)};
    const T s_p = 0.5 * (sgn(mp_p(0)) + sgn(mp_p(1))) *
                  fabs(0.5 * (sgn(mp_p(0)) + sgn(mp_p(2))) * 0.5 *
    ...
    const vec<T, 4> mp_m{4.0 * dj(0) - dj(1), 4.0 * dj(1) - dj(0), dj(0),
                         dj(1)};
    const T s_m = 0.5 * (sgn(mp_m(0)) + sgn(mp_m(1))) *
                  fabs(0.5 * (sgn(mp_m(0)) + sgn(mp_m(2))) * 0.5 *
                       (sgn(mp_m(0)) + sgn(mp_m(3))));
    const T dM4_m =
...
wenozp_reconstruct(T gf_Immm, T gf_Imm, T gf_Im, T gf_Ip, T gf_Ipp, T gf_Ippp,
  ...
  const auto rc_Im{
      wenozp(gf_Immm, gf_Imm, gf_Im, gf_Ip, gf_Ipp, dx, weno_eps, mp)};
  const auto rc_Ip{
      wenozp(gf_Imm, gf_Im, gf_Ip, gf_Ipp, gf_Ippp, dx, weno_eps, mp)};
